define(["dojo/_base/lang", "dojo/_base/array" ,"dojo/_base/declare", 
	...
	declare("dojox.charting.plot2d.__PieCtorArgs", dojox.charting.plot2d.__DefaultCtorArgs, {
		// summary:
		//		Specialized keyword arguments object for use in defining parameters on a Donut chart.
	...
	return declare("dojox.charting.plot2d.Donut", [Base, PlotEvents], {
		// summary:
		...
		clear: function(){
			...
			// returns: dojox/charting/plot2d/Donut
			//		A reference to this plot for functional chaining.
			this.inherited(arguments);
			this.dyn = [];
			this.run = null;
			return this;	//	dojox/charting/plot2d/Donut
		},
		setAxis: function(axis){
			// summary:
			//		Dummy method, since axes are irrelevant with a Donut chart.
			// returns: dojox/charting/plot2d/Donut
		...
		render: function(dim, offsets){
			...
			var _innerradius = r/2;

			if("innerradius" in this.opt){
				_innerradius = this.opt.innerradius;
			}
			...
			arr.some(slices, function(slice, i){
				...
				if(theme.series.fill && theme.series.fill.type === "radial" && this.opt.radGrad === "fan" && step > fanSize){
					...
					for(var j = 0; j < nfans; ++j){
						...
						var fansx1 = j == 0 ? x1 : circle.cx + _innerradius * Math.cos(start + (j - FUDGE_FACTOR ) * delta);
						var fansy1 = j == 0 ? y1 : circle.cy + _innerradius * Math.sin(start + (j - FUDGE_FACTOR ) * delta);
						
						group.createPath().
								moveTo(circle.cx, circle.cy).
								moveTo(fansx1, fansy1).
								lineTo(fansx, fansy).
								arcTo(r, r, 0, delta > Math.PI, true, fanex, faney).
								lineTo(fanex1, faney1).
								arcTo(_innerradius, _innerradius, 0, delta > Math.PI, true, fansx1, fansy1).
								closePath().
					...
					var xx1 = circle.cx + _innerradius * Math.cos(start);
					var yy1 = circle.cy + _innerradius * Math.sin(start);
					var xx2 = circle.cx + _innerradius * Math.cos(end);
					var yy2 = circle.cy + _innerradius * Math.sin(end);
					...
					shape = s.createPath().
						...
						arcTo(r, r, 0, step > Math.PI, true, x2, y2).
						lineTo(xx2, yy2).
						arcTo(_innerradius, _innerradius, 0, step > Math.PI, false, xx1, yy1).
						closePath();
						//moveTo(xx1, yy1).
					//.setStroke(theme.series.stroke);
			...
			// draw a circle and fill to hide the stroke lines inside 
			// draw labels
			if(this.opt.labels){
